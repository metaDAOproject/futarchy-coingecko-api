...
WITH filtered_and_extracted AS (
    ...
    SELECT
        block_time,
        CAST(block_time AS DATE) AS trading_date,
        tx_id,
...
-- Final aggregation by token AND date for daily breakdown
SELECT
    token,
    CAST(trading_date AS VARCHAR) AS date,
    CAST(SUM(CASE WHEN swap_type = 'buy' THEN input_amount ELSE output_amount END) AS VARCHAR) AS base_volume,
    CAST(SUM(CASE WHEN swap_type = 'buy' THEN output_amount ELSE input_amount END) AS VARCHAR) AS target_volume,
